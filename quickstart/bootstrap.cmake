...
if(CMAKE_HOST_WIN32)
  ...
  # A released Visual Studio wins; a preview (Insiders) channel is used only
  # when it is the machine's only C++ toolset, because ConanCenter serves no
  # binaries for a preview toolset and every third-party library would build
  # from source. Among releases vswhere orders by version, so 2026 wins over 2022.
...
# A build tree configured by a different generator (another Visual Studio was
# picked since) cannot be reused; the bootstrap owns these directories, so it
# starts that tree over instead of failing on the cache mismatch.
function(zlink_reset_build_if_generator_changed build)
  if(NOT EXISTS "${build}/CMakeCache.txt")
    return()
  endif()
  file(STRINGS "${build}/CMakeCache.txt" _zlink_cached_generator REGEX "^CMAKE_GENERATOR:INTERNAL=")
  list(GET ZLINK_GENERATOR 1 _zlink_wanted_generator)
  if(NOT _zlink_cached_generator STREQUAL "CMAKE_GENERATOR:INTERNAL=${_zlink_wanted_generator}")
    message(STATUS "generator changed (${_zlink_cached_generator} -> ${_zlink_wanted_generator}); resetting ${build}")
...
function(zlink_build name source install)
  ...
  zlink_reset_build_if_generator_changed("${build}")
...
if(ZLINK_PACKAGE_MANAGER STREQUAL "conan")
  find_program(ZLINK_CONAN conan)
  if(NOT ZLINK_CONAN)
    zlink_fail("could not find Conan. Install it with 'pipx install conan', run 'pipx ensurepath', and open a new terminal. If you used 'pip --user', add Python's Scripts/bin directory to PATH, then verify with 'conan --version'.")
  endif()
...
zlink_reset_build_if_generator_changed("${ZLINK_PROJECT_DIR}/build")
...
# --- 6. the IDE preset ---------------------------------------------------------
# Visual Studio, Rider and CLion configure from CMake presets, so the exact
# arguments this script just used are written once as the user preset
# `zlink` (CMakeUserPresets.json is per-machine and git-ignored). Opening the
# folder after a bootstrap therefore reuses build/ as configured here; the
# preset is regenerated by every bootstrap and is not edited by hand.
...
if(CMAKE_HOST_WIN32)
  set(_zlink_preset_build "      \"configuration\": \"${ZLINK_CONFIG}\"\n")
else()
  set(_zlink_preset_build "      \"jobs\": ${ZLINK_JOBS}\n")
endif()
file(WRITE "${ZLINK_PROJECT_DIR}/CMakeUserPresets.json"
  ...
  "    {\n"
  "      \"name\": \"zlink\",\n"
  "      \"displayName\": \"zlink bootstrap (${ZLINK_CONFIG})\",\n"
  "      \"description\": \"Written by bootstrap.cmake; run it again instead of editing this file\",\n"
...
message(STATUS "bootstrap done. Next: cmake --build \"${ZLINK_PROJECT_DIR}/build\" "
  ...
  "(or open this folder in Visual Studio, Rider or CLion and pick the preset 'zlink')")
